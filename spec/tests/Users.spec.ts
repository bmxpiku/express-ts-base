...
describe('Users Routes', () => {
    ...
    it(`should return a JSON object with all the users and a status code of "${OK}" if the
      ...
      agent.get(getUsersPath)
        .end((err: Error, res: Response) => {
          ...
          const retUsers = res.body.users.map((user: IUser) => new User(user));
  ...
  describe(`"POST:${addUsersPath}"`, () => {
    const callApi = (reqBody: object) => agent.post(addUsersPath).type('form').send(reqBody);
  ...
  describe(`"PUT:${updateUserPath}"`, () => {
    const callApi = (reqBody: object) => agent.put(updateUserPath).type('form').send(reqBody);
  ...
  describe(`"DELETE:${deleteUserPath}"`, () => {
    const callApi = (id: number) => agent.delete(deleteUserPath.replace(':id', id.toString()));
